...
```{r}
df <- read.csv("./puntuacionesFA.csv")
str(df)
```

Como vemos en este caso las columnas con filas las identifica como character, pero va a ser un problema ya que en este caso no vamos a utilizar esas columnas para analizar nada, cuando queramos analizar esas columnas utilizaremos los subsets que ya hemos generado.
...
```{r}
df <- read.csv("./directores_subset.csv")
```
...
Parece que los datos no son normales, además no tenemos una n suficiente como para asumir la teoría del límite central, por lo que en vez de estudiar la homocedastidad podemos pasar directamente a utilizar un test no paramétrico.
```{r}
